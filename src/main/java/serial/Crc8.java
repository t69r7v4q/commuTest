...
public class Crc8 {
    ...
    public static boolean CRC(String crc_str,String crc_res) {
    	String crc8_hex = getCrcRes(crc_str);
//    	System.out.println("res:"+crc8_hex);
    	if (crc8_hex.equals(crc_res)) {
			return true;
		} else {
			return false;
		}
        ...
        //???
    public static String getCrcRes(String crc_str) {
		crc_str = crc_str.replaceAll(" ", "");
    	
    	int crc_8 = calcCrc8(hexStrToBinaryStr(crc_str));
    	String crc8_hex = Integer.toHexString((crc_8 & 0x000000ff) | 0xffffff00) .substring(6).toUpperCase();
		return crc8_hex;
	}
  ...
  //将字符串转为16进制
    ...
    public static void main(String[] args) {
		String str = "00040001000A";
		byte[] data = hexStrToBinaryStr(str);
		int crc_8 = calcCrc8(data);
		System.out.println(crc_8);
		String crc8_hex = Integer.toHexString((crc_8 & 0x000000ff) | 0xffffff00) .substring(6).toUpperCase();
		System.out.println(crc8_hex);//FE
		System.out.println(CRC(str,"CA"));
	}
